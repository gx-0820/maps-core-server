...
@Slf4j
@Component
public class ObjectDetectionVideoStreamHandler extends BinaryWebSocketHandler {

    private final ImageProcessor imageProcessor;
    private final AlgorithmDataProcessor algorithmDataProcessor;
    private static final ConcurrentHashMap<String, Set<WebSocketSession>> deviceFrontendSessions = new ConcurrentHashMap<>();

    public ObjectDetectionVideoStreamHandler(ImageProcessor imageProcessor, AlgorithmDataProcessor algorithmDataProcessor) {
        this.imageProcessor = imageProcessor;
        this.algorithmDataProcessor = algorithmDataProcessor;
    }
    ...
    public void forwardToFrontend(String deviceId, ByteBuffer videoData) {
        ...
        if (sessions.isEmpty()) {
//            log.info("没有找到设备ID对应的前端会话 - DeviceID={}", deviceId);
            return;
        }
        ...
        // 注意：CameraClientHandler已经处理过检测框，此处直接转发接收到的数据
        ByteBuffer processedBuffer = videoData;
            
        sessions.forEach(session -> {
            try {
                if (session.isOpen()) {
                    // 注意：必须复制缓冲区避免竞争
                    ByteBuffer copy = ByteBuffer.allocate(processedBuffer.remaining());
                    copy.put(processedBuffer.slice());
                    copy.flip();
