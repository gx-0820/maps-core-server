...
import com.example.coreserver.grpc.photoelectric.DeviceList;
import com.example.coreserver.service.device.PhotoelectricService;
import com.example.coreserver.service.socket.PhotoelectricClientHandler;
...
@Configuration
@EnableScheduling
public class PhotoelectricClientConfig {

    @Value("${photoelectric.websocket.ws-url}")
    private String serviceAUrl;

    @Autowired
    private PhotoelectricService photoelectricService; // 用于获取设备 ID 的服务

    @Autowired
    private PhotoelectricClientHandler serviceAClientHandler; // WebSocket 处理器
    ...
    @PostConstruct
    public void init() {
        // 尝试从 PhotoelectricService 获取设备 ID
    ...
    /**
     * 从 PhotoelectricService 获取设备 ID
     */
    private String getDeviceIdFromService() {
        try {
            // 获取设备列表
            DeviceList deviceListResponse = photoelectricService.getPhotoelectricDevices();
            if (deviceListResponse != null) {
                // 返回第一个设备 ID
                return deviceListResponse.getDevices(0).getDeviceId();
            }
        } catch (Exception e) {
            log.error("从 PhotoelectricService 获取设备 ID 失败: {}", e.getMessage());
        }
        return null;
    }
